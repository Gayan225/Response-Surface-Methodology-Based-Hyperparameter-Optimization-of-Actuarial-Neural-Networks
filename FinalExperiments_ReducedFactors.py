...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 16:44:10 2024
...
set1 = [4, 6]                   #Optimisers 
#set2 = [15, 25]                 #Number of Neurons for the 1st layer
set3 = [13, 23]                  #Number of Neurons for the 2nd layer
#set4 = [8, 12]                  #Number of Neurons for the 3rd layer
set5 = [503, 903]               #Epoch
set6 = [6542, 10542]            #Batch Size
...
all_sets = [set1, set3, set5, set6, set7]

# Generate the Cartesian product of all sets
combinations = list(product(*all_sets))

# Create a DataFrame to store combinations
columns = ['Optimizer', 'N2', 'Epoch', 'Batch', 'Layer']
experiments = pd.DataFrame(combinations, columns=columns)

experiments.loc[32] = [5, 18, 703, 8542, 3]
experiments.loc[33] = [5, 18, 703, 8542, 3]
experiments.loc[34] = [5, 18, 703, 8542, 3]
experiments.loc[35] = [5, 18, 703, 8542, 3]


# Calculate m and s values
m_values = [(max(s) + min(s)) / 2 for s in [set1, set3, set5, set6, set7]]
s_values = [(max(s) - min(s)) / 2 for s in [set1, set3, set5, set6, set7]]
...
for i, column in enumerate(experiments.columns):
    new_row_pos = {col: (round(a * s_values[i] + m_values[i]) if col == column else m_values[j]) for j, col in enumerate(experiments.columns)}
    new_row_neg = {col: (round(-a * s_values[i] + m_values[i]) if col == column else m_values[j]) for j, col in enumerate(experiments.columns)}
    new_rows.append(new_row_pos)
    new_rows.append(new_row_neg)

# Convert the list of new rows to a DataFrame
new_rows_df = pd.DataFrame(new_rows)

# Concatenate the original DataFrame with the new rows DataFrame
experiments = pd.concat([experiments, new_rows_df], ignore_index=True)
experiments['Optimizer'] = experiments['Optimizer'].round()
...
experiments = experiments.fillna(0).astype(int)


#Decide which method of handling star points.

#experiments.at[36, 'Optimizer'] = 7%7
#experiments.at[36, 'Optimizer'] = 6
...
for i in range(36, 37):
    ...
    q2 = experiments.iloc[i, 1]              # number of neurons in second hidden layer
    q3 = 10             # number of neurons in third hidden layer
    epochs = experiments.iloc[i, 2]
    batch_size = experiments.iloc[i, 3]
    ...
    if experiments.iloc[i, 4] == 2:
        ...
        model_cann.get_layer('Network').set_weights([np.zeros((q1, 1)), np.full((1,), np.log1p(lambda_hom))])
        print('K=1 Layer')
...
experiments.to_csv('Final_Experiments_Reduces_Mod_Corrected.csv', index=False)
