...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 14:08:10 2024
...
s=31.5334
columns = ['Optimizer_c', 'N2_c', 'Epoch_c', 'Batch_c', 'Layer_c']
sequential = pd.DataFrame(columns=columns)

for i in np.arange(0, 0.35, 0.05):
    sequential.loc[i, 'Optimizer_c'] = (-(i+1)/s)*(-21.87026)
    sequential.loc[i, 'N2_c'] = (-(i+1)/s)*(-11.01320)
    sequential.loc[i, 'Epoch_c'] = (-(i+1)/s)*(-16.00502)
    sequential.loc[i, 'Batch_c'] = (-(i+1)/s)*(9.19871)
    sequential.loc[i, 'Layer_c'] = (-(i+1)/s)*(-7.33961)
    
sequential = sequential.apply(pd.to_numeric)
    
sequential['Optimizer_c'] = (s1 * sequential['Optimizer_c'] + m1).round()
#sequential['N1_c'] = (s2 * sequential['N1_c'] + m2).round()
sequential['N2_c'] = (s3 * sequential['N2_c'] + m3).round()
#sequential['N3_c'] = (s4 * sequential['N3_c'] + m4).round()
sequential['Epoch_c'] = (s5 * sequential['Epoch_c'] + m5).round()
sequential['Batch_c'] = (s6 * sequential['Batch_c'] + m6).round()
sequential['Layer_c'] = (s7 * sequential['Layer_c'] + m7).round()

sequential = sequential.astype(int)
sequential = sequential.reset_index(drop=True)
sequential['Optimizer_c']=sequential['Optimizer_c']%7
sequential['loss'] = None
...
for i in range(10):  # range(134) generates numbers from 0 to 133 inclusive
    print(i)
    q1 = 20             # number of neurons in first hidden layer
    q2 = sequential.iloc[i, 1]              # number of neurons in second hidden layer
    q3 = 10             # number of neurons in third hidden layer
    epochs = sequential.iloc[i, 2]
    batch_size = sequential.iloc[i, 3]
    ...
    # Recreate the model with the corrected data
    glm2 = sm.GLM(
        train['ClaimNb'],
    ...
    if sequential.iloc[i, 4] == 2:
        print("K=2 Layer")
    elif sequential.iloc[i, 4] == 3:
        Network = Dense(units=q3, activation='tanh', name='hidden3')(Network)
        print("K=3 Layer")
    elif sequential.iloc[i, 4] == 5:
        Network = Dense(units=q3, activation='tanh', name='hidden3')(Network)
    ...
    model_cann.compile(
        loss='poisson',
        optimizer=optimizers[sequential.iloc[i, 0]]
    )
    ...
    sequential.loc[i, 'loss'] = test_poisson_deviance
    
sequential.to_csv('sequential_red_fact_high_resolution.csv', index=False)
